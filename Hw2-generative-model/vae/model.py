...
class Encoder(nn.Module):
    def __init__(self, input_shape, latent_dim):
        ...
        self.convs = nn.Sequential(
            nn.Conv2d(in_channels=self.input_shape[0], out_channels=32, kernel_size=3, padding=1),
            nn.ReLU(),
        ...
        self.fc = nn.Linear(in_features=self.conv_out_dim, out_features=self.latent_dim)
...
class VAEEncoder(Encoder):
    def __init__(self, input_shape, latent_dim):
        ...
        self.conv_out_dim = encoder.conv_out_dim
        self.fc = nn.Linear(in_features=self.conv_out_dim, out_features=2 * self.latent_dim)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, output_shape):
        ...
        self.deconvs = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=output_shape[0], kernel_size=3, padding=1),
        )
...
if __name__ == "__main__":
    ...
    encoder = Encoder(input_shape, latent_dim)
    emb = encoder(img)
    print(encoder(img).size())

    encoder = VAEEncoder(input_shape, latent_dim)
