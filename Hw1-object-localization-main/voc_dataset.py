...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        rois = self.roi_data['boxes'][0][index][:self.top_n].astype(np.float) \
            if self.roi_data['boxes'][0][index].shape[0] > self.top_n else \
            self.roi_data['boxes'][0][index].astype(np.float)

        proposals = np.zeros(rois.shape)
        proposals[:, 1] = rois[:, 0] / height
        proposals[:, 0] = rois[:, 1] / width
        proposals[:, 3] = rois[:, 2] / height
        proposals[:, 2] = rois[:, 3] / width
