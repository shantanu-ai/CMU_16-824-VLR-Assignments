...
class Trainer:
    ...
    def train_test_loop(self, mode='train', epoch=1000):
        # print(epoch * len(self.data_loaders[mode]) + 0)
        # print(epoch)
        ...
        for step, data in tqdm(enumerate(self.data_loaders[mode])):
            ...
            if mode == 'val':
                ...
                tensor_preds = torch.cat((tensor_preds, pred_class), dim=0)
                gt = torch.cat((gt, answers), dim=0)
            ...
            if mode == 'val' and step == 0:  # change this to show other images
                _n_show = 6  # how many images to plot
                for i in range(_n_show):
                    self.writer.add_image(
                        f"Epoch: {epoch} || Image: {i} || "
                        f"question: {data['question'][i]} || "
                        f"Ground_Truth_answer: {self._id2answer[torch.argmax(answers[i]).item()]} || "
                        f"Predicted_answer: {self._id2answer[torch.argmax(scores[i]).item()]}",
                        data['orig_img'][i].cpu().numpy(),
                        epoch * _n_show + i, dataformats='CHW'
                    )
                    self.writer.add_text(
                        f'Epoch: {epoch} || Input_Question_{i}',
                        data['question'][i],
                        epoch * _n_show + i
                    )
                    self.writer.add_text(
                        f'Epoch: {epoch} || Ground_Truth_answer_{i}',
                        self._id2answer[torch.argmax(answers[i]).item()],
                        epoch * _n_show + i
                    )
                    self.writer.add_text(
                        f'Epoch: {epoch} || Predicted_answer{i}',
                        self._id2answer[torch.argmax(scores[i]).item()],
                        epoch * _n_show + i
                    )
        ...
        torch.save(tensor_preds, f"output.pt")
        torch.save(gt, f"gt.pt")
        return acc
